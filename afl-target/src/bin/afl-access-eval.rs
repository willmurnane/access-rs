...
use access::{AccessToken, AccessTokens, evaluate};
...
fn jvm_evaluate(jvm: &Jvm, expression: &str, tokens: &AccessTokens) -> j4rs::errors::Result<bool> {
    ...
    let tokens_set = jvm.invoke_static(
        "java.util.Set",
        "of",
        &[InvocationArg::try_from(tokens_array).unwrap()],
    )?;
    // Create an Authorizations object
    let auths = jvm.invoke(
        &access,
        "newAuthorizations",
    ...
    let evaluator = jvm.invoke(&access, "newEvaluator", &[InvocationArg::from(auths)])?;
...
fn main() {
    ...
    verify(&jvm, b"\"\xd5\x97(\"\0\"\xd5\x97(\"");
    verify(&jvm, b"\0\"\xd5\x97(\"");
